...
- Trabajarás **en grupos de 2 personas**.
...
| `fecha_registro` | `DATE` | Sí | | Fecha en que el cliente se registró |
...
| `costo_unitario` | | Sí | | Costo de adquisición |
| `stock` | | Sí | DEFAULT 0 | Unidades disponibles en bodega |
...
| `cantidad` | | Sí | CHECK > 0 | |
| `precio_unitario` | `DECIMAL(10,2)` | Sí | | Precio al momento de la venta (puede diferir del precio actual del producto) |
...
| ¿Por qué el `precio_unitario` se guarda en `DETALLE_VENTAS` y no se puede simplemente calcular desde `PRODUCTOS`? | |
